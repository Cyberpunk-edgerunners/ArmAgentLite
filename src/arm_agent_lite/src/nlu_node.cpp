...
public:
  NluNode() : Node("nlu_node") {
    ...
    subscriber_ = this->create_subscription<std_msgs::msg::String>(
        "/command_text", 10,
        [this](const std_msgs::msg::String::SharedPtr msg) { this->processCommand(msg->data); });
    RCLCPP_INFO(this->get_logger(), "NLU Node 已启动，正在监听 /command_text...");
  }
...
private:
  ...
  void processCommand(const std::string& text) {
    ...
    } else if (text.find("原点") != std::string::npos || text.find("home") != std::string::npos) {
      RCLCPP_INFO(this->get_logger(), "[解析结果]: 执行【回原点】动作");
    } else {
      RCLCPP_WARN(this->get_logger(), "[解析警告]: 未知指令，我听不懂你在说什么。");
    }
  }

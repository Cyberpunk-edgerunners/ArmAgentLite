...
public:
  ...
  ArmControllerNode() : Node("arm_controller_node") {
    ...
    action_server_ =
        rclcpp_action::create_server<Grasp>(this, "/grasp_action",
    ...
    RCLCPP_INFO(this->get_logger(), "机械臂控制节点已启动，等待抓取指令(Action)...");
  }
...
private:
  ...
  rclcpp_action::GoalResponse handleGoal(const rclcpp_action::GoalUUID& uuid,
    ...
    (void)uuid;
    RCLCPP_INFO(this->get_logger(), "收到抓取请求！目标坐标: (%.2f, %.2f, %.2f)", goal->target_x,
  ...
  // 回调 2: 处理紧急取消请求
  rclcpp_action::CancelResponse handleCancel(const std::shared_ptr<GoalHandleGrasp> goal_handle) {
  ...
  void handleAccepted(const std::shared_ptr<GoalHandleGrasp> goal_handle) {
    ...
    // 必须开辟一个新线程去执行真正的硬件动作
    std::thread{std::bind(&ArmControllerNode::executeTask, this, _1), goal_handle}.detach();
  }
  ...
  void executeTask(const std::shared_ptr<GoalHandleGrasp> goal_handle) {
    ...
    for (int i = 5; i > 0; --i) {
      ...
      RCLCPP_INFO(this->get_logger(), "发布反馈: 距离目标 %.2f 米", feedback->distance_left);
